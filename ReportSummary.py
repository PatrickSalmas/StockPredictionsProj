...
for dp in dayPred:
    print("^^^^^^^^^^^^^^^^^^")
    print("------------------")
    print("Report Summary for "+str(dp)+" Day Prediction")
    investList = [500,600,700,800,900,1000]
    for i in investList:
        print
        print dateStamp," Investment Value: ",i
        for j in range(0,5):
            path = "C:/Users/psalm/Documents/StockProj/S&P500_GrowthProfiles/"+str(dp)+"DayPred/"+str(i)+"/"+str(j)+"_"+str(j)+"/Overall.txt"
            file = open(path,"r")
            data = file.readlines()
            ...
            path = "C:/Users/psalm/Documents/StockProj/S&P500_GrowthProfiles/"+str(dp)+"DayPred/"+ str(i) + "/" + str(j) + "_" + str(j) + "/SBCount.txt"
            file = open(path,"r")
            data = file.readlines()
